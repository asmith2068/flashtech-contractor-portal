...
// Welcome email sent to a new contractor with their login + a quick tour of the tools.
function welcomeEmailHtml(name, email, tempPw) {
  const nm = String(name || "").replace(/[<>&]/g, "");
  const feat = (emoji, title, desc) => `<tr><td style="padding:7px 0;vertical-align:top;width:34px;font-size:20px">${emoji}</td><td style="padding:7px 0"><b style="color:#111;font-size:14px">${title}</b><br><span style="font-size:13px;color:#555">${desc}</span></td></tr>`;
  const shot = (src, cap) => `<td style="padding:4px;text-align:center;width:33%"><img src="${src}" width="150" style="width:100%;max-width:170px;border:1px solid #e1e4e1;display:block"><div style="font-size:11px;color:#6a7278;margin-top:4px">${cap}</div></td>`;
  const inner = `
    <p style="margin:0 0 12px">Hi ${nm || "there"},</p>
    <p style="margin:0 0 14px">Welcome aboard! Your <b>Flash-Tech Contractor Portal</b> account is ready. Build custom flashings with live 3D previews and instant pricing, browse our full parts catalog, and send quote or order requests — right from your phone, tablet, or computer.</p>
    <div style="background:#f2f3f2;border:1px solid #e1e4e1;border-left:4px solid #0DD714;padding:14px 16px;margin:0 0 18px">
      <div style="font-size:11px;color:#6a7278;text-transform:uppercase;letter-spacing:.05em;margin-bottom:6px">Your Login</div>
      <div style="font-size:15px;margin-bottom:3px"><b>Username:</b> ${email}</div>
      <div style="font-size:15px"><b>Temporary password:</b> <span style="font-family:Consolas,monospace;background:#fff;border:1px solid #d5d8d5;padding:2px 9px">${tempPw}</span></div>
      <div style="font-size:12px;color:#6a7278;margin-top:9px">🔒 For your security, you'll be asked to create your own password the first time you sign in.</div>
    </div>
    <div style="text-align:center">${mailBtn("Open the Portal &amp; Sign In", PORTAL_URL)}</div>
    <p style="text-align:center;font-size:12px;color:#888;margin:6px 0 22px">${PORTAL_URL.replace("https://", "")}</p>

    <div style="font-family:Arial;font-weight:bold;text-transform:uppercase;letter-spacing:.04em;color:#111;font-size:14px;border-bottom:2px solid #0DD714;padding-bottom:6px;margin-bottom:4px">What you can do</div>
    <table style="width:100%;border-collapse:collapse;margin-bottom:16px">
      ${feat("📐", "Custom Flashing Builder", "Pipe boots, wraps, scuppers, drip edge, coping, box caps &amp; more — with a 3D model, part number &amp; price.")}
      ${feat("✏️", "Draw-Your-Own Profiles", "Sketch any bent metal profile by finger or mouse and get instant pricing.")}
      ${feat("📦", "Full Parts Catalog", "Every Flash-Tech part with photos and current pricing.")}
      ${feat("📸", "Snap to Identify", "Take a photo of a flashing and the app helps identify the type.")}
      ${feat("📄", "Quotes &amp; Orders", "Send requests, message our team, print quotes, and track everything.")}
    </table>
    <table style="width:100%;border-collapse:collapse;margin-bottom:20px"><tr>
      ${shot(PORTAL_URL + "/email-home.png", "Browse & build")}
      ${shot(PORTAL_URL + "/email-builder.png", "3D flashing builder")}
      ${shot(PORTAL_URL + "/email-profile.png", "Draw your own profile")}
    </tr></table>

    <div style="font-family:Arial;font-weight:bold;text-transform:uppercase;letter-spacing:.04em;color:#111;font-size:14px;border-bottom:2px solid #0DD714;padding-bottom:6px;margin-bottom:8px">Add it to your device — no app store needed</div>
    <table style="width:100%;font-size:13.5px;color:#333">
      <tr><td style="padding:5px 0"><b>iPhone / iPad (Safari):</b> Share button → Add to Home Screen → Add</td></tr>
      <tr><td style="padding:5px 0"><b>Android (Chrome):</b> tap Install when it pops up (or ⋮ menu → Install app)</td></tr>
      <tr><td style="padding:5px 0"><b>Computer (Chrome / Edge):</b> click the install icon in the address bar</td></tr>
    </table>
    <p style="font-size:13px;color:#666;margin-top:18px">Questions? Just reply to this email or call us at (619) 334-9491.</p>`;
  return emailShell("Welcome to the Contractor Portal", inner);
}
...
// First-login forced password change (after an admin sends a temporary password).
function ForcePasswordChange({ user, onDone }) {
  ...
  return (
    <div className="login"><div className="lbox">
      <img src="/ft-logo.png" alt="Flash-Tech Mfg, Inc." style={{ width: "100%", maxWidth: 320, display: "block", margin: "0 auto 8px" }} />
      <h1 style={{ fontSize: 20 }}>Set Your <span>Password</span></h1>
      <div style={{ color: "var(--mut)", fontSize: 14, margin: "6px 0 14px" }}>Welcome{user.name ? `, ${user.name}` : ""}! Create your own password to finish setting up your account.</div>
      <form onSubmit={submit}>
        {err && <div className="err">{err}</div>}
        <div className="fld"><label>New Password</label><input type="password" value={pw} onChange={(e) => setPw(e.target.value)} autoFocus /></div>
        <div className="fld"><label>Confirm Password</label><input type="password" value={pw2} onChange={(e) => setPw2(e.target.value)} /></div>
        <button className="btn btn-p" style={{ width: "100%", justifyContent: "center" }} disabled={busy}>{busy ? "Saving…" : "Save & Continue"}</button>
      </form>
    </div></div>
  );
}
